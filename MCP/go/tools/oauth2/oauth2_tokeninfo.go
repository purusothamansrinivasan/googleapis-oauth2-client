...
func Oauth2_tokeninfoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["id_token"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("id_token=%v", val))
		}
		...
		url := fmt.Sprintf("%s/oauth2/v2/tokeninfo%s", cfg.BaseURL, queryString)
...
func CreateOauth2_tokeninfoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_oauth2_v2_tokeninfo",
		mcp.WithDescription(""),
		...
		mcp.WithString("access_token", mcp.Description("")),
		mcp.WithString("id_token", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Oauth2_tokeninfoHandler(cfg),
	}
}
